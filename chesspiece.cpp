...
void ChessPiece::mousePressEvent(QGraphicsSceneMouseEvent *event)
{
    ...
    if(!game->selectedPiece && !this->m_promotionPiece) {
        ...
        game->selectedPiece->currentField()->setBrush(QColor(219, 158, 35));
...
bool ChessPiece::colorAvailableFields(Field *field)
{
    if(!field->isItFree()) {
        ...
        if(k) {
            field->setBrush(QColor(46, 215, 184));
        } else {
            field->setBrush(QColor(225, 229, 21));
        }
        return true;
    }
    m_possibleMoves.last()->setBrush(QColor(194, 200, 10));
    return false;
}
